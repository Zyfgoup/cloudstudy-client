        ...
        <el-form label-width="120px">
          ...
          <el-form-item label="头像">
              <el-upload
                         ...
                         action="http://localhost:8888/api/client/user/user/avatar/upload">
    ...
    methods: {
      save(){
        loginApi.updataMemberInfo(this.memberInfo)
          .then(response => { //修改成功
            //提示成功
            cookie.set('cloudstudy_ucenter',JSON.stringify(response.data.data),{domain: 'localhost'})
            this.$message({
              type: 'success',
              message: '修改成功！ 🧙‍♂️'
            });
      ...
      handleAvatarSuccess(response) {
        if (response.date.code === 200) {
          this.memberInfo.avatar = response.data.data
          // 强制重新渲染
          this.$forceUpdate()
        } else {
      ...
      showInfoFromCookie() {
        //从cookie中获取用户信息
        var userInfo = cookie.get("cloudstudy_ucenter")
        this.memberInfo = JSON.parse(userInfo);
      },
      ...
      wxLogin() {
        ...
        loginApi.getMemberInfoSelf(userInfo.id).then(response => {
